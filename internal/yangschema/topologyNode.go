...
func initTopologyNode(p *yentry.Entry, opts ...yentry.EntryOption) *yentry.Entry {
	children := map[string]yentry.EntryInitFunc{
		"state": initTopologyNodeState,
		"tag":   initTopologyNodeTag,
	}
	e := &yentry.Entry{
		Name: "node",
		Key: []string{
		...
		LeafRefs: []*leafref.LeafRef{
			{
				...
				RemotePath: &gnmi.Path{
					Elem: []*gnmi.PathElem{
						{Name: "topology"},
						{Name: "kind", Key: map[string]string{"name": ""}},
					},
				},
			},
		},
