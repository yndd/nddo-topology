...
const (
	// Errors
	errUnexpectedTopologyNode       = "the managed resource is not a TopologyNode resource"
	errKubeUpdateFailedTopologyNode = "cannot update TopologyNode"
	errReadTopologyNode             = "cannot read TopologyNode"
	errCreateTopologyNode           = "cannot create TopologyNode"
	erreUpdateTopologyNode          = "cannot update TopologyNode"
	errDeleteTopologyNode           = "cannot delete TopologyNode"

	// resource information
	// resourcePrefixTopologyNode = "topo.nddo.yndd.io.v1alpha1.TopologyNode"
)

// SetupTopologyNode adds a controller that reconciles TopologyNodes.
func SetupTopologyNode(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) (string, chan cevent.GenericEvent, error) {

	name := managed.ControllerName(topov1alpha1.TopologyNodeGroupKind)

	events := make(chan cevent.GenericEvent)

	y := initYangTopologyNode()

	r := managed.NewReconciler(mgr,
		resource.ManagedKind(topov1alpha1.TopologyNodeGroupVersionKind),
		managed.WithExternalConnecter(&connectorTopologyNode{
			log:         nddcopts.Logger,
			kube:        mgr.GetClient(),
		...
		managed.WithValidator(&validatorTopologyNode{
			log:        nddcopts.Logger,
			rootSchema: nddcopts.Yentry,
			y:          y,
	...
	return topov1alpha1.TopologyNodeGroupKind, events, ctrl.NewControllerManagedBy(mgr).
		Named(name).
		WithOptions(o).
		For(&topov1alpha1.TopoTopologyNode{}).
...
func initYangTopologyNode(opts ...yresource.Option) yresource.Handler {
	r := &TopologyNode{&yresource.Resource{}}
...
func (r *TopologyNode) GetRootPath(mg resource.Managed) []*gnmi.Path {
	// json unmarshal the resource
	cr, ok := mg.(*topov1alpha1.TopoTopologyNode)
	if !ok {
		return []*gnmi.Path{}
	}
	...
	return []*gnmi.Path{
		{
			Elem: []*gnmi.PathElem{
				...
				{Name: "node", Key: map[string]string{
					"name": *cr.Spec.ForNetworkNode.TopoTopologyNode.Name,
				}},
			},
		},
	}
...
func (r *TopologyNode) GetParentDependency(mg resource.Managed) []*leafref.LeafRef {
...
type validatorTopologyNode struct {
	log        logging.Logger
...
func (v *validatorTopologyNode) ValidateLocalleafRef(ctx context.Context, mg resource.Managed) (managed.ValidateLocalleafRefObservation, error) {
	return managed.ValidateLocalleafRefObservation{
		Success:          true,
		ResolvedLeafRefs: []*leafref.ResolvedLeafRef{}}, nil
}

func (v *validatorTopologyNode) ValidateExternalleafRef(ctx context.Context, mg resource.Managed, cfg []byte) (managed.ValidateExternalleafRefObservation, error) {
	...
	cr, ok := mg.(*topov1alpha1.TopoTopologyNode)
	if !ok {
		return managed.ValidateExternalleafRefObservation{}, errors.New(errUnexpectedTopologyNode)
	}
	d, err := json.Marshal(&cr.Spec.ForNetworkNode.TopoTopologyNode)
	if err != nil {
...
func (v *validatorTopologyNode) ValidateParentDependency(ctx context.Context, mg resource.Managed, cfg []byte) (managed.ValidateParentDependencyObservation, error) {
	log := v.log.WithValues("resource", mg.GetName())
	log.Debug("ValidateParentDependency...")

	o, ok := mg.(*topov1alpha1.TopoTopologyNode)
	if !ok {
		return managed.ValidateParentDependencyObservation{}, errors.New(errUnexpectedTopologyNode)
	}
...
func (v *validatorTopologyNode) ValidateResourceIndexes(ctx context.Context, mg resource.Managed) (managed.ValidateResourceIndexesObservation, error) {
	log := v.log.WithValues("resource", mg.GetName())

	// json unmarshal the resource
	cr, ok := mg.(*topov1alpha1.TopoTopologyNode)
	if !ok {
		return managed.ValidateResourceIndexesObservation{}, errors.New(errUnexpectedTopologyNode)
	}
...
// is called.
type connectorTopologyNode struct {
	log         logging.Logger
	kube        client.Client
...
func (c *connectorTopologyNode) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	...
	return &externalTopologyNode{client: cl, targets: tns, log: log, parser: *parser.NewParser(parser.WithLogger(log)), rootSchema: c.rootSchema, y: c.y}, nil
}

// An ExternalClient observes, then either creates, updates, or deletes an
// external resource to ensure it reflects the managed resource's desired state.
type externalTopologyNode struct {
...
func (e *externalTopologyNode) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	cr, ok := mg.(*topov1alpha1.TopoTopologyNode)
	if !ok {
		return managed.ExternalObservation{}, errors.New(errUnexpectedTopologyNode)
	}
	...
	if err != nil {
		return managed.ExternalObservation{}, errors.Wrap(err, errReadTopologyNode)
	}
	...
	var s topov1alpha1.NddotopologyTopologyNode
	...
	cr.Status.AtNetworkNode.TopoTopologyNode = &s
...
func (e *externalTopologyNode) Create(ctx context.Context, mg resource.Managed) (managed.ExternalCreation, error) {
	cr, ok := mg.(*topov1alpha1.TopoTopologyNode)
	if !ok {
		return managed.ExternalCreation{}, errors.New(errUnexpectedTopologyNode)
	}
	log := e.log.WithValues("Resource", cr.GetName())
	log.Debug("Creating ...")
	...
	if err != nil {
		return managed.ExternalCreation{}, errors.Wrap(err, errCreateTopologyNode)
	}

	return managed.ExternalCreation{}, nil
}

func (e *externalTopologyNode) Update(ctx context.Context, mg resource.Managed, obs managed.ExternalObservation) (managed.ExternalUpdate, error) {
	cr, ok := mg.(*topov1alpha1.TopoTopologyNode)
	if !ok {
		return managed.ExternalUpdate{}, errors.New(errUnexpectedTopologyNode)
	}
	log := e.log.WithValues("Resource", cr.GetName())
	log.Debug("Updating ...")
	...
	_, err := e.client.Set(ctx, req)
	if err != nil {
		return managed.ExternalUpdate{}, errors.Wrap(err, errReadTopologyNode)
	}

	return managed.ExternalUpdate{}, nil
}

func (e *externalTopologyNode) Delete(ctx context.Context, mg resource.Managed) error {
	cr, ok := mg.(*topov1alpha1.TopoTopologyNode)
	if !ok {
		return errors.New(errUnexpectedTopologyNode)
	}
	log := e.log.WithValues("Resource", cr.GetName())
	log.Debug("Deleting ...")
	...
	_, err := e.client.Set(ctx, &req)
	if err != nil {
		return errors.Wrap(err, errDeleteTopologyNode)
	}

	return nil
}

func (e *externalTopologyNode) GetTarget() []string {
	return e.targets
}

func (e *externalTopologyNode) GetConfig(ctx context.Context) ([]byte, error) {
...
func (e *externalTopologyNode) GetResourceName(ctx context.Context, path []*gnmi.Path) (string, error) {
	return "", nil
}
