...
const (
	// Errors
	errUnexpectedTopologyLink       = "the managed resource is not a TopologyLink resource"
	errKubeUpdateFailedTopologyLink = "cannot update TopologyLink"
	errReadTopologyLink             = "cannot read TopologyLink"
	errCreateTopologyLink           = "cannot create TopologyLink"
	erreUpdateTopologyLink          = "cannot update TopologyLink"
	errDeleteTopologyLink           = "cannot delete TopologyLink"

	// resource information
	// resourcePrefixTopologyLink = "topo.nddo.yndd.io.v1alpha1.TopologyLink"
)

// SetupTopologyLink adds a controller that reconciles TopologyLinks.
func SetupTopologyLink(mgr ctrl.Manager, o controller.Options, nddcopts *shared.NddControllerOptions) (string, chan cevent.GenericEvent, error) {

	name := managed.ControllerName(topov1alpha1.TopologyLinkGroupKind)

	events := make(chan cevent.GenericEvent)

	y := initYangTopologyLink()

	r := managed.NewReconciler(mgr,
		resource.ManagedKind(topov1alpha1.TopologyLinkGroupVersionKind),
		managed.WithExternalConnecter(&connectorTopologyLink{
			log:         nddcopts.Logger,
			kube:        mgr.GetClient(),
		...
		managed.WithValidator(&validatorTopologyLink{
			log:        nddcopts.Logger,
			rootSchema: nddcopts.Yentry,
			y:          y,
	...
	return topov1alpha1.TopologyLinkGroupKind, events, ctrl.NewControllerManagedBy(mgr).
		Named(name).
		WithOptions(o).
		For(&topov1alpha1.TopoTopologyLink{}).
...
func initYangTopologyLink(opts ...yresource.Option) yresource.Handler {
	r := &TopologyLink{&yresource.Resource{}}
...
func (r *TopologyLink) GetParentDependency(mg resource.Managed) []*leafref.LeafRef {
...
type validatorTopologyLink struct {
	log        logging.Logger
...
func (v *validatorTopologyLink) ValidateLocalleafRef(ctx context.Context, mg resource.Managed) (managed.ValidateLocalleafRefObservation, error) {
	return managed.ValidateLocalleafRefObservation{
		Success:          true,
		ResolvedLeafRefs: []*leafref.ResolvedLeafRef{}}, nil
}

func (v *validatorTopologyLink) ValidateExternalleafRef(ctx context.Context, mg resource.Managed, cfg []byte) (managed.ValidateExternalleafRefObservation, error) {
...
func (v *validatorTopologyLink) ValidateParentDependency(ctx context.Context, mg resource.Managed, cfg []byte) (managed.ValidateParentDependencyObservation, error) {
	log := v.log.WithValues("resource", mg.GetName())
	log.Debug("ValidateParentDependency...")

	o, ok := mg.(*topov1alpha1.TopoTopologyLink)
	if !ok {
		return managed.ValidateParentDependencyObservation{}, errors.New(errUnexpectedTopologyLink)
	}
...
func (v *validatorTopologyLink) ValidateResourceIndexes(ctx context.Context, mg resource.Managed) (managed.ValidateResourceIndexesObservation, error) {
	log := v.log.WithValues("resource", mg.GetName())
...
// is called.
type connectorTopologyLink struct {
	log         logging.Logger
	kube        client.Client
...
func (c *connectorTopologyLink) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	...
	return &externalTopologyLink{client: cl, targets: tns, log: log, parser: *parser.NewParser(parser.WithLogger(log)), rootSchema: c.rootSchema, y: c.y}, nil
}

// An ExternalClient observes, then either creates, updates, or deletes an
// external resource to ensure it reflects the managed resource's desired state.
type externalTopologyLink struct {
...
func (e *externalTopologyLink) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	...
	if err != nil {
		return managed.ExternalObservation{}, errors.Wrap(err, errReadTopologyLink)
	}
...
func (e *externalTopologyLink) Create(ctx context.Context, mg resource.Managed) (managed.ExternalCreation, error) {
	...
	if err != nil {
		return managed.ExternalCreation{}, errors.Wrap(err, errCreateTopologyLink)
	}
...
func (e *externalTopologyLink) Update(ctx context.Context, mg resource.Managed, obs managed.ExternalObservation) (managed.ExternalUpdate, error) {
	...
	_, err := e.client.Set(ctx, req)
	if err != nil {
		return managed.ExternalUpdate{}, errors.Wrap(err, errReadTopologyLink)
	}
...
func (e *externalTopologyLink) Delete(ctx context.Context, mg resource.Managed) error {
	...
	_, err := e.client.Set(ctx, &req)
	if err != nil {
		return errors.Wrap(err, errDeleteTopologyLink)
	}

	return nil
}

func (e *externalTopologyLink) GetTarget() []string {
	return e.targets
}

func (e *externalTopologyLink) GetConfig(ctx context.Context) ([]byte, error) {
...
func (e *externalTopologyLink) GetResourceName(ctx context.Context, path []*gnmi.Path) (string, error) {
	return "", nil
}
